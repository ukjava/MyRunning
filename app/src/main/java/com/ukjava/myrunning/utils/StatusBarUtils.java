...
public class StatusBarUtils {
    ...
    public static int StatusBarLightMode(Activity activity, boolean isBlack) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (MIUISetStatusBarLightMode(activity, isBlack)) {
                result = 1;
     ...
     * @param mView
     */
    public static void getStateBar(Context mContext, View mView) {
        int result = 0;
        int resourceId = mContext.getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            result = mContext.getResources().getDimensionPixelSize(resourceId);
            mView.getPaddingTop();
            mView.setPadding(mView.getPaddingLeft(), mView.getPaddingTop() + result, mView.getPaddingRight(), mView.getPaddingBottom());
        }
    }
}
